...
void main()
{
    ...
    coord.x = mod(float(texelCoord.x) + t * speed, width*10) / (gl_NumWorkGroups.x * gl_WorkGroupSize.x) * freq;
    coord.y = float(texelCoord.y) / (gl_NumWorkGroups.y * gl_WorkGroupSize.y) * freq;
